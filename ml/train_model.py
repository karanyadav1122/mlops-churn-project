...
with mlflow.start_run(run_name= "logreg_threshold_0_4"):
  ...
  predictions_custom.groupBy("churn_label","prediction_custom").count().show()


  cm = predictions_custom.select(
    when((col("churn_label") ==1) &(col("prediction_custom") == 1),1).otherwise(0).alias("tp"),
    when((col("churn_label")== 0) & (col("prediction_custom") == 0),1).otherwise(0).alias("tn"),
    when((col("churn_label")== 0)& (col("prediction_custom") ==1),1).otherwise(0).alias("fp"),
    when((col("churn_label")==1) & (col("prediction_custom")==0),1).otherwise(0).alias("fn")
  ).agg(
